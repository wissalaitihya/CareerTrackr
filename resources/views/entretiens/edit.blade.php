<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            Modifier l'entretien — {{ $candidature->company_name }}
        </h2>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-3xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white shadow p-6 rounded">
                ...
                <form action="{{ route('entretiens.update', [$candidature, $entretien]) }}"
                      method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="mb-4">
                        ...
                        <select name="type" class="w-full border rounded px-3 py-2 mt-1">
                            @foreach(['Téléphonique', 'Visio', 'Technique', 'RH', 'Sur site'] as $type)
                                <option value="{{ $type }}"
                                    {{ old('type', $entretien->type) == $type ? 'selected' : '' }}>
                                    {{ $type }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-4">
                        ...
                        <input type="datetime-local" name="scheduled_at"
                               value="{{ old('scheduled_at', $entretien->scheduled_at) }}"
                    ...
                    <div class="mb-4">
                        ...
                        <textarea name="preparation_notes" rows="3"
                                  class="w-full border rounded px-3 py-2 mt-1">{{ old('preparation_notes', $entretien->preparation_notes) }}</textarea>
                    </div>

                    <div class="mb-4">
                        <label class="block text-sm font-medium">Résultat</label>
                        <input type="text" name="result"
                               value="{{ old('result', $entretien->result) }}"
                               class="w-full border rounded px-3 py-2 rounded mt-1">
                        @error('result')
                            <p class="text-red-500 text-sm mt-1">{{ $message }}</p>
                        @enderror
                    </div>
