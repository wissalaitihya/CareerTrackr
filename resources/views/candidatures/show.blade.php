                ...
                <div class="p-8">
                    ...
                    <div class="mt-8 pt-6 border-t border-slate-200">
                        ...
                        <div class="flex flex-wrap gap-2">
                            ...
                            @foreach([
                                ['value' => 'postulé', 'label' => 'Postulé', 'color' => 'bg-orange-50 text-orange-700 border-orange-200'],
                                ['value' => 'entretien_rh', 'label' => 'Entretien planifié', 'color' => 'bg-emerald-50 text-emerald-700 border-emerald-200'],
                                ['value' => 'en_attente', 'label' => 'En attente', 'color' => 'bg-amber-50 text-amber-700 border-amber-200'],
                                ['value' => 'offre_reçue', 'label' => 'Offre reçue', 'color' => 'bg-blue-50 text-blue-700 border-blue-200'],
                                ['value' => 'accepté', 'label' => 'Accepté', 'color' => 'bg-emerald-50 text-emerald-700 border-emerald-200'],
                                ['value' => 'refusé', 'label' => 'Refusé', 'color' => 'bg-red-50 text-red-700 border-red-200'],
                            ] as $status)
                                @php $isActive = $currentStatus === $status['value']; @endphp
                                <form action="{{ route('candidatures.update', $candidature) }}" method="POST" class="inline">
                                    ...
                                    <button type="submit" 
                                            ...
                                            @if($isActive) bg-emerald-500 text-white border-emerald-500 shadow-sm
                                            @else bg-white text-gray-600 border-slate-200 hover:bg-gray-50 @endif">
                                        {{ $status['label'] }}
                                    </button>
                                </form>
                            @endforeach
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="p-6 space-y-5">
                            <div>
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Date de candidature</p>
                            ...
                            <div>
                                <p class="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">URL de l'offre</p>
                                <a href="{{ $candidature->offer_url }}" target="_blank" 
                                   class="text-sm text-emerald-600 hover:text-emerald-700 font-medium transition-colors inline-flex items-center gap-1">
                                    <span>{{ preg_replace('/^https?:\/\//', '', $candidature->offer_url) }}</span>
                            ...
                            <div>
                                ...
                                <div class="bg-gray-50 border border-slate-200 rounded-xl p-4">
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
                        <div class="p-6 border-b border-slate-100 flex items-center justify-between">
                            <h2 class="text-lg font-semibold text-gray-900">Entretiens ({{ $candidature->entretiens->count() }})</h2>
                            @if($candidature->entretiens->count() > 0)
                            <a href="#" class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-semibold rounded-lg shadow-sm hover:shadow-md transition-all">
                                <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="p-6">
                            @forelse($candidature->entretiens as $entretien)
                                @php
                                    $dotColor = match($entretien->result) {
                                        'Réussi', 'réussi', 'Réussie', 'réussie' => 'bg-emerald-500',
                                        'En attente', 'en_attente', null => 'bg-amber-400',
                                        'Échec', 'échec', 'Refusé', 'refusé' => 'bg-red-500',
                                        default => 'bg-slate-300',
                                    };
                                    $badgeClass = match($entretien->result) {
                                        'Réussi', 'réussi', 'Réussie', 'réussie' => 'bg-emerald-50 text-emerald-700',
                                        'En attente', 'en_attente', null => 'bg-amber-50 text-amber-700',
                                        'Échec', 'échec', 'Refusé', 'refusé' => 'bg-red-50 text-red-700',
                                        default => 'bg-slate-100 text-slate-700',
                                    };
                                @endphp
                                    ...
                                    <div class="flex items-start justify-between gap-4">
                                        <div class="flex-1">
                                            ...
                                            <p class="text-xs text-gray-400 mb-2">
                                                @if($entretien->schedules_at)
                                                    {{ \Carbon\Carbon::parse($entretien->schedules_at)->translatedFormat('D j F · H:i') }}
                                                @endif
                                            </p>
                                        ...
                                        <div class="flex items-center gap-1 shrink-0">
                                            <a href="#" class="p-1.5 text-gray-400 hover:text-gray-600 transition-colors">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                                                </svg>
                                            </a>
                                            <form action="#" method="POST" onsubmit="return confirm('Supprimer cet entretien ?')">
                                                @csrf @method('DELETE')
                            ...
                            @empty
                                <div class="text-center py-8">
                                    ...
                                    <p class="text-sm font-medium text-gray-500 mb-1">Aucun entretien pour le moment</p>
                                    <p class="text-xs text-gray-400 mb-4">Ajoute un entretien pour suivre le processus</p>
                                    <a href="#" class="inline-flex items-center gap-1.5 px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 text-white text-xs font-semibold rounded-lg shadow-sm hover:shadow-md transition-all">
                                        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                        </svg>
                ...
                <div class="space-y-6">
                        ...
                        <div class="p-5 space-y-4">
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($candidature->created_at)->translatedFormat('j F Y') }} • {{ $daysAgo === 0 ? "Aujourd'hui" : ($daysAgo === 1 ? 'hier' : "il y a $daysAgo jours") }}</p>
                                </div>
                            </div>
                            <div class="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p class="text-xs text-gray-400">{{ \Carbon\Carbon::parse($candidature->updated_at)->translatedFormat('j F Y') }} • {{ now()->diffInDays(\Carbon\Carbon::parse($candidature->updated_at)) === 0 ? "Aujourd'hui" : (now()->diffInDays(\Carbon\Carbon::parse($candidature->updated_at)) === 1 ? 'hier' : 'il y a ' . now()->diffInDays(\Carbon\Carbon::parse($candidature->updated_at)) . ' jours') }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="bg-white border border-slate-200 rounded-2xl shadow-sm">
                        ...
                        <div class="p-5 space-y-2">
                            <a href="#" class="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-xl transition-colors">
                                <svg class="w-4 h-4 text-emerald-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"/>
                                </svg>
                                Ajouter un entretien
                            </a>
                            <a href="{{ route('candidatures.edit', $candidature) }}" class="flex items-center gap-2.5 w-full px-4 py-2.5 text-sm font-medium text-gray-700 hover:bg-gray-50 rounded-xl transition-colors">
