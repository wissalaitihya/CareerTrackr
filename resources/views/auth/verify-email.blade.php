<x-guest-layout>
    ...
    <div class="mb-10">
        ...
        <p class="text-gray-500 text-sm max-w-[280px] leading-relaxed">
            Un lien de vérification a été envoyé à <span
                class="font-medium text-gray-700">{{ Auth::user()->email }}</span>. Clique dessus pour activer ton
            compte.
        </p>
    </div>
    ...
    <div class="flex flex-col gap-3">
        ...
        <form method="POST" action="{{ route('logout') }}">
            @csrf
            <button type="submit"
                class="w-full py-4 px-6 bg-gray-100 text-gray-700 font-semibold text-sm rounded-2xl hover:bg-gray-200 transition-colors">
                Se déconnecter
            </button>
        </form>
    </div>
